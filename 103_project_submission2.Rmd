...
project_plots <- function(df_name, gene_name, cont_covariate, cat_covariate1, cat_covariate2){
  ...
  annotations <- data.frame(
    x = c(round(min(cleanedData[[gene_name]]), 2), round(mean(cleanedData[[gene_name]]), 2), round(max(cleanedData[[gene_name]]), 2)),
    y = c(4, 12, 5),
  ...
  boxplot <- ggplot(cleanedData,aes_string(x = cat_covariate1,y = gene_name,fill = cat_covariate2)) +
    ...
    # Change labels
    labs(x = paste(cat_covariate1),y = paste0(gene_name, ' Gene Frequency'),fill = paste(cat_covariate2),
         title = paste0(gene_name, ' Gene Frequency by ', cat_covariate1, ' and ', cat_covariate2))+
    project_theme
  print(boxplot)
}
